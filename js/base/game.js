...
KMGames.base = async function() {
  ...
  // If player has progress, offer to continue or restart
  if (capitulo > 1) {
    E.clear();
    E.print('Você tem progresso salvo!', 'bold');
    E.print(`Último capítulo: ${capitulo}`);
    const choice = await E.choice([
      {label: 'Continuar', value: 'continue', primary: true},
  ...
  if (bossWon && finalSecreto) {
    ...
    await E.sleep(2000);
    // Let player retry from exploration
    S.setBaseCapitulo(5);
    await E.anyKey();
    return;
  }
  ...
  // Game complete — reset chapter for replay
  S.setBaseCapitulo(7);
  await E.sleep(3000);
};
